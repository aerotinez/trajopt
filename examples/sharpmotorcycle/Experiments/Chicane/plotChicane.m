...
saveas(fig,fig_path + "results_chicane",'epsc');
...
view(axe_road,-90,5);
xlim(axe_road,[0,rd.Data(1,end)]);
ylim(axe_road,[rd.Data(2,1) - 7,rd.Data(2,end) + 7]);
zlim(axe_road,[0,3]);
box(axe_road,"on");
camproj(axe_road,"perspective");
title(axe_road,"Chicane scenario");
daspect(axe_road,[1,1,1]);
...
function fig = plotRoad(fig,road,result,color)
    ...
    Y = [p0(2,:);p(2,:)];
    Z = [p0(3,:);p(3,:)];
    surf(X,Y,Z,"FaceColor",color,"EdgeColor","none","FaceAlpha",0.1);
